...
public class Scoreboard {
    ...
    public Scoreboard(Player player) {
        ...
        org.bukkit.scoreboard.Scoreboard board = LibraryPlugin.getInstance().getServer().getScoreboardManager().getNewScoreboard();
    ...
    public void update() {
        String untranslatedTitle = LibraryPlugin.getInstance().getScoreboardHandler().getConfiguration().getTitleGetter().getTitle(this.player);
        String title = Color.translate(untranslatedTitle);

        List<String> lines = this.localList.get();
        if (!lines.isEmpty()) lines.clear();
        LibraryPlugin.getInstance().getScoreboardHandler().getConfiguration().getScoreGetter().getScores(this.localList.get(), this.player);
        this.recentlyUpdatedScores.clear();
        this.usedBaseScores.clear();
        int nextValue = lines.size();

        if (lines.size() > 16) {
            LibraryPlugin.getInstance().getLogger().warning("Too many lines passed!");
            return;
        }

        if (title.length() > 32) {
            LibraryPlugin.getInstance().getLogger().warning("Title is too long!");
            return;
        }
    ...
    private String[] separate(String line, Collection<String> usedBaseScores) {
        ...
        switch (this.separateScores.size()) {
            ...
            case 3:
                ...
                break;
            default:
                LibraryPlugin.getInstance().getLogger().warning("Failed to separate scoreboard line. Input: " + line);
                break;
        }
        ...
        if (usedBaseScores.contains(score)) {
            if (score.length() <= 14) {
                ...
                if (usedBaseScores.contains(score)) {
                    LibraryPlugin.getInstance().getLogger().warning("Failed to find alternate color code for: " + score);
                }
            } else {
                LibraryPlugin.getInstance().getLogger().warning("Found a scoreboard base collision to shift: " + score);
            }
        }
