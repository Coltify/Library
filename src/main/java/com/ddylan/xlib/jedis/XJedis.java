package com.ddylan.xlib.jedis;

import com.ddylan.xlib.Library;
import com.ddylan.xlib.redis.RedisCommand;
import com.ddylan.xlib.serialization.*;
...
public final class XJedis {
    ...
    public XJedis() {
        try {
            localJedisPool = new JedisPool(new JedisPoolConfig(), Library.getInstance().getConfig().getString("Redis.Host"), 6379, 20000, null, Library.getInstance().getConfig().getInt("Redis.DbId", 0));
        } catch (Exception e) {
            localJedisPool = null;
            e.printStackTrace();
            Library.getInstance().getLogger().warning("Couldn't connect to the Redis pool - '" + Library.getInstance().getConfig().getString("Redis.Host") + ":6379'");
        }

        try {
            backboneJedisPool = new JedisPool(new JedisPoolConfig(), Library.getInstance().getConfig().getString("BackboneRedis.Host"), 6379, 20000, null, Library.getInstance().getConfig().getInt("BackboneRedis.DbId", 0));
        } catch (Exception e) {
            backboneJedisPool = null;
            e.printStackTrace();
            Library.getInstance().getLogger().warning("Couldn't connect to the Backbone Redis pool - '" + Library.getInstance().getConfig().getString("BackboneRedis.Host") + ":6379'");
        }
    }
