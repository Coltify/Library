...
public class PlayerInventorySerializer
{
    public static String serialize(final Player player) {
        return Library.getInstance().getXJedis().PLAIN_GSON.toJson(new PlayerInventoryWrapper(player));
    }
    
    public static PlayerInventoryWrapper deserialize(final String json) {
        return Library.getInstance().getXJedis().PLAIN_GSON.fromJson(json, PlayerInventoryWrapper.class);
    }
