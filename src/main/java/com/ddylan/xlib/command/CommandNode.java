...
import com.ddylan.xlib.util.Stopwatch;
    ...
    public boolean invoke(CommandSender sender, Arguments arguments) throws CommandException {
        ...
        for (Data unknownData : this.parameters) {
            ...
            if (unknownData instanceof ParameterData) {
                ...
                ParameterType<?> type = Library.getInstance().getCommandHandler().getParameterType(parameterData.getType());
        ...
        try {
            ...
            stopwatch.stop();
            int executionThreshold = Library.getInstance().getConfig().getInt("Command.TimeThreshold", 10);
            if (!this.async && this.logToConsole && stopwatch.elapsed() >= executionThreshold)
                Library.getInstance().getLogger().warning("Command '/" + getFullLabel() + "' took " + stopwatch.elapsed() + "ms!");
            return true;
        ...
        if (canUse(sender)) {
            ...
            if (this.parent == null) {
                commands.add(command);
                    ...
                    Library.getInstance().getCommandHandler().ROOT_NODE.getCommand(this.parent.getName()) != this.parent) {
                commands.add(command);
            }
            if (hasCommands())
